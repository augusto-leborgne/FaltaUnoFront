...
export const PlayerProfile: FC<PlayerProfileProps> = ({ player }) => {
      ...
      <div className="flex-1 px-6 py-6">
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center space-x-4 mb-4">
            <Badge className="bg-orange-100 text-gray-800 hover:bg-orange-100">{player.position}</Badge>
            <div className="flex items-center space-x-1">
              <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
              <span className="font-semibold">{player.rating}</span>
              <span className="text-gray-500">({player.totalMatches} partidos)</span>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="text-center bg-gray-50 rounded-xl p-4">
              <div className="text-2xl font-bold text-gray-900">{player.stats.assists}</div>
              <div className="text-sm text-gray-600">Asistencias</div>
            </div>
            <div className="text-center bg-gray-50 rounded-xl p-4">
              <div className="text-2xl font-bold text-gray-900">{player.totalMatches}</div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div className="text-center">
                    ...
                    <div className="flex justify-center space-x-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-2.5 h-2.5 ${i < review.nivel ? "fill-blue-400 text-blue-400" : "text-gray-300"}`} />
                      ))}
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="flex justify-center space-x-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-2.5 h-2.5 ${i < review.deportividad ? "fill-green-400 text-green-400" : "text-gray-300"}`} />
                      ))}
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="flex justify-center space-x-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-2.5 h-2.5 ${i < review.companerismo ? "fill-purple-400 text-purple-400" : "text-gray-300"}`} />
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <div className="pb-24">
          <Button onClick={handleBack} variant="outline" className="w-full py-4 text-lg font-semibold rounded-2xl border-gray-300 bg-transparent">
            Volver
          </Button>
        </div>
      </div>
